...
{
    int i;
    printf("\n in forward_client_message\n");
    for (i = 0; i < 10; i++)
    {
        if(!strcmp(cli_dbase[i].nick_name, client->rem_name)) {
          ...
          printf("\n found clinet index %d", i);
          return msg_encode_client_buff(buff, client->rem_name, client->msg_buff);
        }
        else
            return 0;
    }
...
int add_new_client_entry(client_t *new_client, struct sockaddr_in *rem_addr)
{
    ...
    for (i = 0; i < 10; i++)
    {
        if(!strcmp(cli_dbase[i].nick_name, new_client->nick_name))
            return REJECTED;
        if (dbase_count < 10)
        {
